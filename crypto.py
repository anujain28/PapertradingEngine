...
# --- PAGE: MANUAL BOT (RESTORED INPUTS) ---
def show_crypto_manual_bot_page():
    ...
    with c2:
        ...
        grids = col_b.number_input("No. of Grids", value=5, min_value=2)
...
# --- PAGE: AUTO PILOT (RESTORED INPUTS) ---
def show_ai_autopilot_page():
    ...
    if not ap["running"]:
        ...
        # 1. Currency Selection (Restored)
        curr_mode = c1.radio("Capital Currency", ["USDT (USD)", "INR (₹)"])
        
        # 2. Capital Input (Restored)
        ...
        if ap['active_grids']:
            for i, g in enumerate(ap['active_grids']):
                ...
                with st.expander(f"Orders for {g['coin']}"):
                    if 'orders' in g:
                        st.table(pd.DataFrame(g['orders']))
